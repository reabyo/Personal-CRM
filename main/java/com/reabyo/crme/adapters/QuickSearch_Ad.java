...
public class QuickSearch_Ad extends BaseAdapter {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        if (view == null){
            view = LayoutInflater.from(context).inflate(R.layout.quick_search_list_item, parent, false);
            holder = new Holder();
        ...
        if (position == 0 || position % 2 == 0){
            view.setBackgroundResource(R.drawable.row1_style);
        }
        else if (position % 2 == 1 ){
            view.setBackgroundResource(R.drawable.row2_style);
        }
        return view;
    }
